...
static void aligment_l_file(t_file **file, t_out *ip) {
...
void mx_print_l_file_detail(t_file **file) {
    ...
    for (int i = 0; file[i]; i++) {
        if (file[i]->name[0] != '.') {
            ...
            file[i]->driver ? mx_left_aligment(file[i]->driver->driver,
            ip->count_size) : mx_left_aligment(ip->size, ip->count_size);
