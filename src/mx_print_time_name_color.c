...
static void print_uname_gr_size(t_file *file, t_flags *flags, t_out *ip) {
    if (flags->f_g == 0)
        mx_right_alt(file->username, ip->count_uname);
    if (flags->f_o == 0)
        mx_right_alt(file->gr ? file->gr : ip->gid, ip->count_group);
    if (file->driver)
        mx_left_aligment(file->driver->driver, ip->count_size);
    else
...
void mx_print_time_name_color(t_file *file, time_t this_time,
    ...
    print_uname_gr_size(file, flags, ip);
